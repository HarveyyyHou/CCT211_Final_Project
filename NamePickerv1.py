...
class Name_Pick:
    ...
    def get_settings_screen_existing_values(self):
        if self.textbox_rigged_names:
            self.rigged_names = self.textbox_rigged_names.get("1.0","end-1c")

    def get_main_screen_existing_values(self):
        if self.textbox_userinput and self.textbox_choosen_name:
            self.userinput = self.textbox_userinput.get("1.0","end-1c")
            self.choosen_name = self.textbox_choosen_name.get("1.0","end-1c")
    ...
    def get_random_data(self):
        ...
        if len(rigged)!=0:
            ...
            if picked_name in userinput_names:
                ...
                self.textbox_userinput = ",".join(userinput_names)
